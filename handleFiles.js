...
const handleFiles = async ( fileList, target = null, formattingFunction = null ) => {
	let text = await extractFileText(fileList)

	if(formattingFunction && text ){
		text = formattingFunction(text)
	}
...
window.addEventListener("drop", (event) => {
	...
	if(target && files ){
		handleFiles(files, target, beautifyJSON)
	}
},false)
